...
%     betas=0.8;
    ...
    betas=0.745346887723725;
    ls=0.302644871949323;
    Be=0.518007176648594;
    Rs=0.064292146073037;
end
...
% ave_beta=sum(2*Pr(ind)./max(Pr).*rr(ind)*dr);
ave_beta=sum(2*Pr(ind)./max(Pr).*rr(ind)*dr)/sum(2.*rr(ind)*dr);
tot_I=pi*sum(2*Jt(ind).*rr(ind)*dr);
tot_I2=pi*sum(2*Jt.*rr*dr);

Izeta0=tot_I2*((Be*Rs)/(2*mu0))
% Izeta0=tot_I2;

% load('gs1d.mat');

subplot(221);
plot(rr*Rs*100,psi.*(Be*Rs^2),'k--','linewidth',2);hold all;
set(gca,'Fontsize',15);
xlabel('r (cm)');ylabel('\psi');xlim([0 100*Rw]);
% hx=legend('$solve\_MRR$','$GS1D$');set(hx,'interpreter','latex');legend('boxoff');
title(['\beta_s=',num2str(betas),', l_s=',num2str(ls),', l_{sp}=',num2str(lsp,3)]);

subplot(222);
plot(rr*Rs*100,Pr.*(Be^2/(2*mu0)),'k--','linewidth',2);
set(gca,'Fontsize',15);
xlabel('r (cm)');ylabel('P');xlim([0 100*Rw]);
% hx=legend('$solve\_MRR$','$GS1D$');set(hx,'interpreter','latex');legend('boxoff');
title(['l_{s0}=',num2str(ls0),', \alpha=',num2str(alpha),', \psi_g=',num2str(psig)]);

subplot(223);
plot(rr*Rs*100,Jt.*(Be/(2*mu0*Rs)),'k--','linewidth',2);
set(gca,'Fontsize',15);
xlabel('r (cm)');ylabel('J_\theta');xlim([0 100*Rw]);
% hx=legend('$solve\_MRR$','$GS1D$');set(hx,'interpreter','latex');legend('boxoff');
title(['\sigma=',num2str(sigma),', h=',num2str(h),', n=',num2str(n)]);

subplot(224);
plot(rr*Rs*100,Bz.*Be,'k--','linewidth',2);
set(gca,'Fontsize',15);
xlabel('r (cm)');ylabel('B_z');xlim([0 100*Rw]);
% hx=legend('$solve\_MRR$','$GS1D$');set(hx,'interpreter','latex');legend('boxoff');
...
if (1==1)
    save('MRR1d.mat','betas','ls','alpha','sigma','psibc','n','Rs','Rw','psi1d','pr1d',...
        'j1d','Bz1d','rr1d','C1s','C2s','n','Be','Rs','Rw','Izeta0'); 
end
