...
figure('unit','normalized','DefaultAxesFontSize',fontsize,...
    ...
    'position',[0.1,0.2,0.28,0.3],'color', [1, 1, 1]);
% Bz_plasma=Bz(end,:);
% psi_plasma=psi(end,:);
...
% contour(Z,R,psii,30,'linewidth',2); hold on;colorbar('east');colormap('hot'); 
...
% xlabel('$z (m)$','interpreter','latex','FontSize',20);
...
% 
% h=rectangle('Position',[-0.75 0.147 0.1 0.006]);w = h.LineWidth;h.LineWidth = 3;hold on;
% h=rectangle('Position',[-0.60 0.147 1.2 0.006]);w = h.LineWidth;h.LineWidth = 3;hold on;
% h=rectangle('Position',[0.65 0.147 0.1 0.006]);w = h.LineWidth;h.LineWidth = 3;hold on;
...
% xlim([0,100*rm]);set(gca,'xtick',[0:2.5:100*rm]);ylim([0,4e5]);
...
% text('Interpreter','latex','String','$(d)$','Position',[1 3.5e5],'color','k','FontSize',fontsize)
...
% h=legend('$2D$','$1D$');set(h,'interpreter','latex');legend('boxoff');
% xlim([0,100*rm]);set(gca,'xtick',[0:2.5:100*rm]);ylim([0,5e7]);
% set(gca,'GridLineStyle',':','GridColor','k','GridAlpha',0.25);
% set(gcf, 'PaperPositionMode', 'auto');
% text('Interpreter','latex','String','$(e)$','Position',[1 4.5e7],'color','k','FontSize',fontsize)

% load('psib_paper.mat');% read data
...
% r_x=floor(0.14/dR);
% % plot(Z(r_x,:),psi(r_x,:),'c-','linewidth',3);grid on;
% plot(Z(r_x,:),psi(r_x,:),'-','color',[0.5 0 0],'linewidth',3);grid on;
% set(gca,'Fontsize',fontsize);
...
% h=legend('$r=0.14$');set(h,'interpreter','latex');
% legend('boxoff');
% text('Interpreter','latex','String','$(b)$','Position',[-0.7 0.035],'color','k','FontSize',fontsize)

% axes('position',[0.58,0.41,0.38,0.26]); %fig-3
for i=1:size(Z,1)
    Bi(i)=interp1(Z(i,:),Bz(i,:),-0.72);
end
% plot(zb,Bz,'r-',Z(1,:),Bz_plasma,'g--','linewidth',3);grid on;hold all;
% plot(Z(r_x,:),Bz(r_x,:),'c-','linewidth',3);grid on;hold all;
% plot(Z(r_x,:),Bz(r_x,:),'-','color',[0.5 0 0],'linewidth',3);grid on;hold all;

% plot(R(:,3)*100,Bz(:,3),'k-','linewidth',3);grid on;hold all;
axes('position',[0.11,0.14,0.85,0.83]);%fig-1 [left bottom width height]
plot(R(:,3)*100,Bi,'m-','linewidth',3);grid on;hold all;
set(gca,'Fontsize',fontsize);
% ylabel('$B_{z} \ at \ wall$','interpreter','latex','FontSize',fontsize);
ylabel('$B_{z} \ [T]$','interpreter','latex','FontSize',fontsize);
xlabel('$r [cm]$','interpreter','latex','FontSize',fontsize);
% xlim([-0.75,0.75]);set(gca,'xtick',[-0.6:0.4:0.6]);set(gca,'ytick',[0:2:10]);
xlim([0,14]);set(gca,'xtick',[0:2:14]);
ylim([2,6]);set(gca,'ytick',[2:1:6]);
set(gca,'GridLineStyle',':','GridColor','k','GridAlpha',0.25);
% h=legend('$\mathbf{w/o \ plasma}$','$\mathbf{w \ plasma}$');set(h,'interpreter','latex');
% h=legend('$r=0.14$');set(h,'interpreter','latex');
h=legend('$\mathbf{fixed-boundary}$');set(h,'interpreter','latex','FontSize',28);
legend('boxoff');
