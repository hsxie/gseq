...
a=0.15; zm=0.75*zw; ra=0.45*rw; rb=0.8*rw;
% rr(ind1)=0.15*(tanh(5*(zz(ind1)+0.8*zw))+1.0)+0.4;
wall=2;
if(wall==1)
    rr(ind1)=0.5*(rb-ra)*(tanh(80*(zz(ind1)+zm))+1.0)+ra;
    rr(ind2)=0.5*(rb-ra)*(tanh(80*(zm-zz(ind2)))+1.0)+ra;
else
%     rr=0.67*rw+0.*zz;
    rr=0.14+0.*zz;
end
...
save('psib_paper.mat','zb','rb','psib','Bz');
save('../psib_paper.mat','zb','rb','psib','Bz');
Z0=(size(psi,2)+1)/2;
fprintf('psib=%17.16f \n',psi(Z0));
